# contracts/c2626/engine/generator_2626.py — Contrato 2626
# Salinas — Pipeline completo (scan + build)
...
from .scanner_2626 import build_content_from_root
from .word_builder_2626 import inserir_conteudo
...
def run_all(
    ...
    """Pipeline completo do contrato 2626 (Salinas): varre pasta → monta conteudo[] → gera .docx"""
    ...
    log_errors = os.path.join(pasta_saida, "c2626_erros_pastas.txt")
    log_process = os.path.join(pasta_saida, "c2626_process_log.txt")
    ...
    with open(log_process, "w", encoding="utf-8") as f:
        f.write("PROCESS LOG — c2626 Salinas\n\n")
