# contracts/c6122/engine/generator_6122.py — Contrato 6122
# Mato Grosso do Sul — Pipeline completo (scan + build)
...
from .scanner_6122 import build_content_from_root
from .word_builder_6122 import inserir_conteudo
...
def run_all(
    ...
    """Pipeline completo do contrato 6122 (Mato Grosso do Sul): varre pasta → monta conteudo[] → gera .docx"""
    ...
    log_errors = os.path.join(pasta_saida, "c6122_erros_pastas.txt")
    log_process = os.path.join(pasta_saida, "c6122_process_log.txt")
    ...
    with open(log_process, "w", encoding="utf-8") as f:
        f.write("PROCESS LOG — c6122 Mato Grosso do Sul\n\n")
    ...
    inserir_conteudo(modelo_path, conteudo, output_docx, meta=meta)
    total_images = sum(1 for item in conteudo if isinstance(item, dict) and "imagem" in item)

    file_logger(f"[OK] Relatório gerado: {output_docx}")
    file_logger(f"[OK] Total de imagens: {total_images}")

    return RunResult(output_docx=output_docx, log_process=log_process, log_errors=log_errors, total_images=total_images)
