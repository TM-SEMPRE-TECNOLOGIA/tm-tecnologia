# contracts/c2627/engine/generator_2627.py — Contrato 2627
# Governador Valadares — Pipeline completo (scan + build)
...
def run_all(
    ...
    """Pipeline completo do contrato 2627 (Governador Valadares): varre pasta → monta conteudo[] → gera .docx"""
    ...
    log_errors = os.path.join(pasta_saida, "c2627_erros_pastas.txt")
    log_process = os.path.join(pasta_saida, "c2627_process_log.txt")
    ...
    with open(log_process, "w", encoding="utf-8") as f:
        f.write("PROCESS LOG — c2627 Governador Valadares\n\n")
